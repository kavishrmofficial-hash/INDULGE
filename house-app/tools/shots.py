#!/usr/bin/env python3
"""Screenshot the review build: every screen at phone size in both colour schemes, plus the desktop stage.
Usage: python3 tools/shots.py [outdir]   (default shots/)  ->  outdir/light/*.png, outdir/dark/*.png"""
import os, sys
...
def shoot(b, scheme):
    ...
    for name, view, param, unlocked in SCREENS:
        ...
        print(scheme, name)
    ctx.close()
    d = b.new_context(viewport={'width': 1280, 'height': 940}, device_scale_factor=1, color_scheme=scheme)
    ...
    page.reload(); page.wait_for_timeout(1400)
    page.screenshot(path=os.path.join(out, 'desktop-stage.png')); print(scheme, 'desktop-stage')
    d.close()

def main():
    with sync_playwright() as p:
        b = p.chromium.launch()
        shoot(b, 'light'); shoot(b, 'dark')
        b.close()
