...
def mock_part(h):
    ...
    # the bubble rides with the pointer, and holds still while the mouse is over it
    p.keyboard.press('Escape')
    p.locator('.buddy-home').click()
    p.wait_for_selector('#buddy-input')
    p.mouse.move(300, 200); p.wait_for_timeout(500)
    b1 = p.evaluate('() => { const r = document.querySelector(".buddy-bubble").getBoundingClientRect(); return [r.left, r.top]; }')
    p.mouse.move(700, 500); p.wait_for_timeout(700)
    b2 = p.evaluate('() => { const r = document.querySelector(".buddy-bubble").getBoundingClientRect(); return [r.left, r.top]; }')
    check(abs(b2[0] - b1[0]) > 150 and abs(b2[1] - b1[1]) > 100, 'bubble should glide with the pointer %r %r' % (b1, b2))
    p.mouse.move(b2[0] + 60, b2[1] + 40); p.wait_for_timeout(500)
    b3 = p.evaluate('() => { const r = document.querySelector(".buddy-bubble").getBoundingClientRect(); return [r.left, r.top]; }')
    check(abs(b3[0] - b2[0]) < 60 and abs(b3[1] - b2[1]) < 60, 'bubble should hold still under the mouse %r %r' % (b2, b3))
    p.keyboard.press('Escape')
